...
class Conf:
    ...
    def get_incomming_path(self) -> path:
        res = path.join(
                self.get_photolab_path(),
                "Incomming"
        )
        if not path.isdir(res):
            makedirs(res)
    ...
    def get_incomming_raw_path(self) -> path:
        res = path.join(
                self.get_incomming_path(),
                "RAW"
        )
        if not path.isdir(res):
            makedirs(res)
    ...
    def get_incomming_jpeg_path(self) -> path:
        res = path.join(
                self.get_incomming_path(),
                "JPEG"
        )
    ...
    def get_workdir_path(self) -> path:
        res = path.join(
                self.get_photolab_path(),
                "Workspace"
        )
    ...
    def get_JPEG_selection_path(self) -> path:
        res = path.join(
    ...
    def get_RAW_selection_path(self) -> path:
        res = path.join(
                self.get_workdir_path(),
                "RAW_selection"
        )
